...
class LMSTestLabSDF:
    ...
    def reconstruct_rpm_from_raw_tacho(
        self,
        raw_channel: str = 'raw:Tacho1',
        pulses_per_revolution: float = 12.0,
        window_s: float = 1.0,
    ) -> tuple[np.ndarray, np.ndarray]:
        """
        Reconstruct RPM from raw tacho event timestamps using pulse counts per time window.

        RPM = (pulse_count / window_s) * 60 / PPR

        This is intentionally window-based rather than based on each individual
        delta_t. The supplied raw tacho trace contains closely spaced trigger events;
        a pulse-count window is much more robust for this file family.
        ...
        rpm : np.ndarray
            Windowed RPM estimate.

        Notes
        -----
        * This is an OPTIONAL derived signal.
        * The original raw:Tacho and Tacho1 channels are never modified.
        * PPR is measurement-setup dependent and must be confirmed by the user.
        * window_s controls time resolution vs robustness; 1.0 s is the default.
        """
        ...
        info = self.channel(raw_channel)
        if info.sampling_type != 'event_timestamps':
            raise ValueError(f'{raw_channel} is not an event timestamp channel.')

        t = self.read_channel(raw_channel).astype(np.float64)
        t = t[np.isfinite(t)]
        if t.size < 2:
            raise SDFFormatError(f'{raw_channel} contains fewer than two valid timestamps.')

        # Remove any non-monotonic duplicate/backward events before histogramming.
        t = np.sort(t)
        start_s = np.floor(t[0] / window_s) * window_s
        end_s = t[-1]
        edges = np.arange(start_s, end_s + window_s, window_s, dtype=np.float64)
        if edges.size < 2:
            edges = np.asarray([start_s, start_s + window_s], dtype=np.float64)
        elif edges[-1] <= end_s:
            edges = np.append(edges, edges[-1] + window_s)

        counts, _ = np.histogram(t, bins=edges)
        time_s = (edges[:-1] + edges[1:]) * 0.5
        rpm = (counts.astype(np.float64) / window_s) * 60.0 / float(pulses_per_revolution)

        return time_s.astype(np.float64, copy=False), rpm.astype(np.float32, copy=False)
    ...
    def to_hdf5(
        ...
        reconstruct_rpm: bool = False,
        raw_tacho_channel: str = 'raw:Tacho1',
        pulses_per_revolution: float = 12.0,
        rpm_window_s: float = 1.0,
    ) -> Path:
        ...
        with h5py.File(output_path, 'w') as h5:
            ...
            # Optional derived RPM reconstruction. This does NOT replace or modify
            # the source raw:Tacho1 or Tacho1 datasets.
            if reconstruct_rpm:
                time_s, rpm = self.reconstruct_rpm_from_raw_tacho(
                    raw_channel=raw_tacho_channel,
                    pulses_per_revolution=pulses_per_revolution,
                    window_s=rpm_window_s,
                )
                gderived = h5.create_group('derived')
                grpm = gderived.create_group('reconstructed_rpm')
                grpm.attrs['source_channel'] = raw_tacho_channel
                grpm.attrs['unit'] = 'rpm'
                grpm.attrs['time_unit'] = 's'
                grpm.attrs['pulses_per_revolution'] = float(pulses_per_revolution)
                grpm.attrs['window_s'] = float(rpm_window_s)
                grpm.attrs['method'] = 'RPM = (pulse_count / window_s) * 60 / PPR'
                grpm.attrs['derived_signal'] = True
                grpm.attrs['source_channels_modified'] = False

                dchunk = max(1, min(len(rpm), 262_144))
                grpm.create_dataset(
                    'time',
                    data=time_s,
                    dtype=np.float64,
